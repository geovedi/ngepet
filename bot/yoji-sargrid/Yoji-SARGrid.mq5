...
#property description "Yoji-SARGrid"

#define EA_NAME "Yoji-SARGrid"
...
input int trendPeriod = 50;                       // Trend MA Period

///

int atrHandle, sarHandle, smaHandle, entryBar;

int OnInit() {
  atrHandle = iATR(Symbol(), trendTimeframe, 20);
  sarHandle = iSAR(Symbol(), entryTimeframe, 0.02, 0.2);
  smaHandle = iMA(Symbol(), trendTimeframe, trendPeriod, 0, //
...
void CreateNewGrid(string symbol, ulong magic) {
  ...
  if (!GetArray(atrHandle, 0, 0, 3, atr) || //
      !GetArray(sarHandle, 0, 0, 3, sar) || //
      !GetArray(smaHandle, 0, 0, 3, sma) || //
      !GetRates(symbol, entryTimeframe, 3, rates))
    return;
  ...
  // entry logic
  bool buySignal = rates[1].close > sar[1] && //
                   rates[1].close > sma[1];
  bool sellSignal = rates[1].close < sar[1] && //
                    rates[1].close < sma[1];
