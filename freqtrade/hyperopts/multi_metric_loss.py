...
class MultiMetricLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(
        ...
        **kwargs
        ...
        # Internal function to calculate SQN
        def calculate_sqn(results: DataFrame) -> float:
            mean = results["profit_abs"].mean()
            std = results["profit_abs"].std()
            return (mean / std) * np.sqrt(len(results))
        ...
        normalized_max_drawdown = normalize_metric(
